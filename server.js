...
app.get("/status-info", (req, res) => {
  try {
    ...
    if (!code) {
      return res.status(400).send({ message: "Invalid or missing status code. Provide a valid HTTP status code in the query parameter." });
    }
    if(!statusCodes[code]){
      return res.status(400).send({ message: "Invalid or missing status code. Provide a valid HTTP status code in the query parameter." });
    }
    res.status(200).send({ status: code, message: statusCodes[code] });
  } catch (error) {
